function [fill_pixelpoints,curvalid,angle]=fill_basicunit_adjacent_r(label_micro,Curbasic_unit,k4,minbox_diffclass,basic_cen,k)
%[fill_pixelpoints]=fill_basicunit_adjacent(label_micro,Curbasic_unit, willfill_basicid,minbox_diffclass);
...
%rotate image
edges = edge(label_micro, 'Canny');
if ~isempty(find(edges))
    [y, x] = find(edges);
    points = [x, y]; 
    coeff = pca(points);
    angle = atan2d(coeff(2, 1), coeff(1, 1));
else
    angle = 90;
end
current_basiclabelr=imrotate(current_basiclabel,angle);   
current_basiclabelr(all(current_basiclabelr==0,2),:)=[];
current_basiclabelr(:,all(current_basiclabelr==0,1))=[];
...
window_size=size(current_basiclabelr); %mask_label_micro上当前需填充的区域尺寸信息
if any(window_size>size(label_micro,1))
    angle=90;
    current_basiclabelr=imrotate(current_basiclabel,angle);  
    window_size=size(current_basiclabelr);
end
...
if ismember(adjacent_class,curexit_class) & length(adjacent_class)==1%影像中存在basic unit的邻接关系
    for row=1: step_row:s-window_size(1)+1
        for col = 1: step_col:s-window_size(2)+1
            ...
            fillimage_info=double(currentl_fillblock)+double(current_basiclabelr);
            ...
            cal_valid_adjacent= (currentl_fillblock==adjacent_class) & (current_basiclabelr==curclass);
            Cal_adjacentnum=[Cal_adjacentnum sum(cal_valid_adjacent(:)==1)];
            %获取当前类别是否与同类像素重叠，若重叠则剔除
            cal_exitsamebasic= (currentl_fillblock==curclass) & (current_basiclabelr==curclass);
